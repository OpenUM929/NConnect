...
# The outer launcher re-enters this same file inside tmux.  It used to name
# server_run_go2_candidate_staged.sh, the ancestor this runner was copied from:
# harmless while a campaign wrapper called it with --inner, fatal the moment this
# script is the entry point (defect C-14).  Re-enter by path, never by a literal name.
...
# Some screening plans require a valid but stationary policy to be measured on
# every mandatory target before it is rejected.  Keep the historical default
# (early stop) unless the package opts in explicitly.  Training non-finiteness
# and an unreadable/unexecutable evaluation are never made collectable by this
# flag: those paths still stop and package the evidence that is safe to keep.
COLLECT_REQUIRED_ON_STATIONARY=${COLLECT_REQUIRED_ON_STATIONARY:-0}
[[ "$COLLECT_REQUIRED_ON_STATIONARY" == 0 || "$COLLECT_REQUIRED_ON_STATIONARY" == 1 ]] || {
  echo '[FAIL] COLLECT_REQUIRED_ON_STATIONARY must be 0 or 1'; exit 2;
}
...
package_result() {
  local label=$1 collection=${2:-UNKNOWN}
  mkdir -p "$KEEP"
  # RESULT_STATE says whether packaging finished; COLLECTION_STATUS says what was
  # actually measured.  They are different questions and a reader must not have to
  # infer one from the other.
  printf 'RESULT_STATE=%s\nCOLLECTION_STATUS=%s\nPACKAGED_AT=%s\n' \
    "$label" "$collection" "$(date -Is)" >"$KEEP/RESULT_STATUS.txt"
...
if [[ "${1:-}" != "--inner" ]]; then
  ...
  [[ -f "$SELF" ]] || { echo "[FAIL] cannot re-enter itself: $SELF is missing"; exit 2; }
  printf -v INNER_COMMAND "cd %q && PACKAGE_ROOT=%q GO2_RESUME=%q GO2_REMEASURE_BASELINE=%q GO2_STAGE=%q ISAACLAB_SH=%q bash %q --inner" \
    "$PACKAGE_ROOT" "$PACKAGE_ROOT" "$RESUME" "$REMEASURE" "$STAGE" "$ISAACLAB_SH" "$SELF"
...
on_exit() {
  ...
  if [[ "$rc" != 0 ]]; then
    trap - EXIT
    printf 'RUNNER_RC=%s\nFAILED_AT=%s\nCOLLECTION_STATUS=INCOMPLETE_CRASH\n' \
      "$rc" "$(date -Is)" >"$KEEP/RUNNER_STATUS.txt"
    package_result PARTIAL INCOMPLETE_CRASH || true
  fi
}
trap on_exit EXIT
...
catastrophe_action() {
  local decision=$1
  case "$decision" in
    SUITE_COMPLETE) echo CONTINUE ;;
    CATASTROPHE_STATIONARY)
      if [[ "$COLLECT_REQUIRED_ON_STATIONARY" == 1 ]]; then
        echo CONTINUE_REQUIRED
      else
        echo STOP_WITH_WITNESS
      fi
      ;;
    *) echo STOP_UNSAFE ;;
  esac
...
finish() {
  ...
  local report_status collection
  local cand_cases base_cases sent_cases cand_videos base_videos
  local exp_cand exp_base exp_sent exp_cand_videos exp_base_videos
  report_status=$(sed -n 's/^REPORT_STATUS=//p' "$KEEP/exported/REPORT_STATUS.txt" 2>/dev/null || true)
  cand_cases=$(count_files "$KEEP/evaluation/candidate/cases" summary.json)
  base_cases=$(count_files "$KEEP/evaluation/$BASELINE_LABEL/cases" summary.json)
  sent_cases=$(count_files "$KEEP/evaluation/${BASELINE_LABEL}_sentinel/cases" summary.json)
  cand_videos=$(count_files "$KEEP/evaluation/candidate/videos" '*.mp4')
  base_videos=$(count_files "$KEEP/evaluation/${BASELINE_LABEL}_videos/videos" '*.mp4')
  # Ready to download and collected in full are different states.  This function is
  # reached both by a run that measured its whole list and by the catastrophe gate's
  # early stop, which measures nothing; both package a ZIP and print the same done
  # marker, so the marker alone cannot tell them apart.  Name the collection state
  # here, from the counts on disk rather than from the decision alone: a gap can only
  # be filled while the volatile server is still alive.
  case "$decision" in
    SUITE_COMPLETE) exp_cand=69 ;;
    TARGET_STAGE_COMPLETE|CATASTROPHE_STATIONARY_MANDATORY_COLLECTION_COMPLETE)
      exp_cand=$(( ${#TARGET_CASES[@]} + 1 )) ;;
    *) exp_cand=NONE ;;
  esac
  if [[ "$REMEASURE" == 1 ]]; then
    exp_base=$exp_cand; exp_sent=0
  else
    exp_base=0; exp_sent=${#SENTINEL_CASES[@]}
  fi
  exp_cand_videos=${#CANDIDATE_VIDEOS[@]}
  exp_base_videos=${#BASELINE_VIDEOS[@]}
  if [[ "$exp_cand" == NONE ]]; then
    collection=INCOMPLETE_EARLY_STOP
  elif [[ "$cand_cases" == "$exp_cand" && "$base_cases" == "$exp_base" && "$sent_cases" == "$exp_sent" &&
          "$cand_videos" == "$exp_cand_videos" && "$base_videos" == "$exp_base_videos" ]]; then
    if [[ "$decision" == SUITE_COMPLETE ]]; then
      collection=FULL_69_COMPLETE
    else
      collection=TARGET_STAGE_COMPLETE_NOT_FULL_69
    fi
  else
    collection=INCOMPLETE_COLLECTION
  fi
  printf 'RUNNER_RC=0\nCOMPLETED_AT=%s\nWORK_ID=%s\nRUN_ID=%s\nDECISION=%s\nSINGLE_CHANGE=%s:%s->%s\nTRAIN_SEED=%s\nCANDIDATE_MODEL_SHA=%s\nCANDIDATE_ENV_SHA=%s\nBASELINE_NAME=%s\nBASELINE_MODEL_SHA=%s\nBASELINE_ARM=%s\nTELEMETRY_CANDIDATE=%s\nTELEMETRY_BASELINE=%s\nTELEMETRY_BASELINE_SENTINEL=%s\nVIDEOS_CANDIDATE=%s\nVIDEOS_BASELINE=%s\nREPORT_STATUS=%s\nEVALUATOR=posture_gate_v2\nEVALUATOR_SHA=%s\nREGISTRY_SHA=%s\nTELEMETRY_SCHEMA=6\nVIDEO_STATUS=VIDEO_UNKNOWN\nOFFICIAL_RESULT=OFFICIAL_RESULT_UNMEASURED\n' \
    ...
    "$cand_cases" "$base_cases" "$sent_cases" "$cand_videos" "$base_videos" \
    "${report_status:-UNKNOWN}" "$EVALUATOR_SHA" "$REGISTRY_SHA" >"$KEEP/RUNNER_STATUS.txt"
  printf 'STAGE=%s\nCOLLECTION_STATUS=%s\n' "$STAGE" "$collection" >>"$KEEP/RUNNER_STATUS.txt"
  printf 'COLLECTION_EXPECTED=candidate:%s baseline:%s sentinel:%s videos:%s baseline_videos:%s\n' \
    "$exp_cand" "$exp_base" "$exp_sent" "$exp_cand_videos" "$exp_base_videos" >>"$KEEP/RUNNER_STATUS.txt"
  ...
  package_result FULL "$collection"
  trap - EXIT
  printf '[COLLECTION] %s candidate=%s/%s baseline=%s/%s sentinel=%s/%s videos=%s/%s baseline_videos=%s/%s\n' \
    "$collection" "$cand_cases" "$exp_cand" "$base_cases" "$exp_base" "$sent_cases" "$exp_sent" \
    "$cand_videos" "$exp_cand_videos" "$base_videos" "$exp_base_videos"
  if [[ "$collection" != FULL_69_COMPLETE && "$collection" != TARGET_STAGE_COMPLETE_NOT_FULL_69 ]]; then
    echo "[INCOMPLETE COLLECTION] decision=$decision — the ZIP below is ready to download but is NOT a full harvest."
    echo '[INCOMPLETE COLLECTION] the missing cases and videos exist only while this server lives: recover, then decide.'
  fi
  echo "$DONE_MARKER"
  exit 0
}
...
cd "$CANDIDATE_ROOT"
# BEGIN INTERRUPTED TRAINING GUARD (defect C-27)
# A resume whose preserved training is not complete used to fall through to the
# retrain branch, which deletes logs/, exported/, the pinned checkpoint and the
# training log.  "Continue where it stopped" then cost the evidence AND the GPU
# hours.  Evidence first, stop second, restart only when asked for by name.
restart_would_destroy() {
  [[ -s "$KEEP/logs/candidate_training.log" || -s "$PIN_MODEL" || -s "$PIN_RECORD" ]] && return 0
  [[ -n "$(find "$CANDIDATE_ROOT/logs" -type f -name 'model_*.pt' -print -quit 2>/dev/null)" ]]
}
preserve_interrupted_training() {
  local dest
  dest="$KEEP/training/interrupted_$(date +%Y%m%d-%H%M%S)"
  mkdir -p "$dest"
  [[ ! -s "$KEEP/logs/candidate_training.log" ]] || cp -a "$KEEP/logs/candidate_training.log" "$dest/"
  [[ ! -s "$PIN_MODEL" ]] || cp -a "$PIN_MODEL" "$dest/"
  [[ ! -s "$PIN_RECORD" ]] || cp -a "$PIN_RECORD" "$dest/"
  for name in model_best.pt env.yaml policy.pt report.html; do
    [[ ! -s "$CANDIDATE_ROOT/exported/$name" ]] || cp -a "$CANDIDATE_ROOT/exported/$name" "$dest/"
  done
  [[ ! -d "$CANDIDATE_ROOT/logs" ]] || cp -a "$CANDIDATE_ROOT/logs" "$dest/logs"
  ( cd "$dest" && find . -type f ! -name SHA256SUMS.txt -print0 | sort -z \
      | xargs -0 sha256sum >SHA256SUMS.txt )
  printf 'TRAINING_STATE=INTERRUPTED_TRAINING_PRESERVED\nPRESERVED_AT=%s\nPRESERVED_IN=%s\nWHY=%s\nTO_RESTART=%s\n' \
    "$(date -Is)" "$dest" \
    "GO2_RESUME=1 but the preserved training is not TRAIN_RC=0; retraining here would delete it" \
    "GO2_RESTART_TRAINING=1 (deletes the preserved training and trains from scratch)" \
    >"$KEEP/training/TRAINING_RESUME_STATUS.txt"
  cat "$KEEP/training/TRAINING_RESUME_STATUS.txt"
}
...
if [[ "$RESUME" == 1 && -s "$KEEP/training/model_best.pt" && -s "$KEEP/training/env.yaml" ]] && \
  ...
  if [[ "$RESUME" == 1 && "${GO2_RESTART_TRAINING:-0}" != 1 ]] && restart_would_destroy; then
    preserve_interrupted_training
    echo '[STOP] INTERRUPTED_TRAINING_PRESERVED: resume cannot continue this training and will not'
    echo '       destroy it.  Download the arm as it stands, or rerun with GO2_RESTART_TRAINING=1'
    echo '       to retrain from scratch over the preserved copy.'
    exit 9
  fi
  rm -rf -- logs exported
  mkdir -p exported
...
COLLECTION_ACTION=$(catastrophe_action "$DECISION")
if [[ "$COLLECTION_ACTION" == CONTINUE_REQUIRED ]]; then
  echo '[MANDATORY COLLECTION] valid stationary policy: collect every configured target/protection case and required video before rejecting performance'
elif [[ "$COLLECTION_ACTION" != CONTINUE ]]; then
  echo "[EARLY STOP] $DECISION — mandatory collection is unsafe or not enabled; preserve one witness only when the policy executed"
  IFS=: read -r GATE_SCENARIO GATE_CASE GATE_SEED <<<"$CATASTROPHE_CASE"
  # A non-finite training trace is not a valid executable policy.  Do not risk
  # another simulator launch merely to render it; the training/checkpoint logs
  # are the safe evidence.  A valid stationary policy retains the historical
  # one-witness behavior when the package did not opt into mandatory collection.
  if [[ "$COLLECTION_ACTION" == STOP_WITH_WITNESS ]]; then
...
collect_candidate_target_cases() {
  ...
  [[ "$(find "$CAND_OUT/cases" -name summary.json | wc -l)" == "$(( ${#TARGET_CASES[@]} + 1 ))" ]] || {
    echo '[FAIL] candidate target-stage telemetry count mismatch'; return 6;
  }
}

collect_configured_videos() {
  # These lists are evidence requirements, not rewards for a passing policy.
  # Failed and stationary-but-executable candidates must leave the same listed
  # views as ordinary target-stage runs when mandatory collection is enabled.
...
if [[ "$STAGE" == target ]]; then
  ...
  collect_candidate_target_cases || exit $?
  if [[ "$DECISION" == CATASTROPHE_STATIONARY ]]; then
    STAGE_DECISION=CATASTROPHE_STATIONARY_MANDATORY_COLLECTION_COMPLETE
  else
