...
#
# G-A023 "Chain-01 baseline".  Rule 8 scores every scenario as
# survival x tracking, and rule 7 puts 0.60 of the Go2 weight on terrain the
# campaign has never measured on Chain-01 with a valid survival metric
# (G3 0.20, G4 0.15, G5 0.15, G7 0.10).  Chain-01 is the only Go2 checkpoint
# with an independently verified, zero-survival-regression single-variable
# result (G-A011, track_lin_vel_xy_exp 1.0->1.2), but it has only ever been
# scored by the tier-1 proxy (1 seed, early-kill cutoff).  Three individual
# reward overlays on top of it have since failed the tier-1 gate, and the
# remaining two participant-file dials were already exhausted before the
# Chain-01 reset.  Spending more GPU hours guessing at a fourth overlay
# before Chain-01's own real score exists would be buying an answer to a
# question we cannot yet ask.
#
# This package therefore trains nothing.  It evaluates the frozen Chain-01
# checkpoint under the posture-gated evaluator across all 69 cases and 3
# seeds, and films one video per scenario.  Cost is about one hour of the
# remaining budget.
#
# Phases
#   1  full 69-case G1-G7 suite on frozen Chain-01, seeds 101/202/303
#   2  one video per scenario (G1-G7) so the numbers have a witness
#   3  one-file packaging
set -euo pipefail

PACKAGE_ROOT=${PACKAGE_ROOT:-/workspace/go2_chain01_baseline}
TRAIN_ROOT="$PACKAGE_ROOT/candidate"
CHAIN01_ROOT="$PACKAGE_ROOT/chain01"
REGISTRY="$PACKAGE_ROOT/go2_self_eval_registry.json"
KEEP=/workspace/_keep/go2_chain01_baseline
RESULT_ZIP=/workspace/_keep/GO2_CHAIN01_BASELINE_RESULT.zip
TMUX_NAME=go2_chain01_baseline
SEEDS=(101 202 303)
EVAL_STEPS=${GO2_EVAL_STEPS:-1000}
VIDEO_STEPS=${GO2_VIDEO_STEPS:-500}
RESUME=${GO2_RESUME:-0}
CHAIN01_EXPECTED_SHA=143871e3f69514a47ea4929c312895cf2da2e95b311aef83209866b3c3e542d4
...
if [[ "${1:-}" != "--inner" ]]; then
  ...
  [[ -d "$TRAIN_ROOT" && -d "$CHAIN01_ROOT" && -s "$REGISTRY" ]] || {
  ...
  tmux new-session -d -s "$TMUX_NAME" \
    "cd '$PACKAGE_ROOT' && PACKAGE_ROOT='$PACKAGE_ROOT' GO2_RESUME='$RESUME' bash server_run_go2_chain01_baseline.sh --inner"
  ...
  echo "[DONE_MARKER] [DONE] GO2_CHAIN01_BASELINE_RESULT_READY"
  exit 0
fi
...
sha256sum "$CHAIN01_ROOT/go2_eval_telemetry.py" >"$KEEP/meta/evaluator.sha256"
PLANE=(
...
echo '[PHASE 1/3] full 69-case suite on frozen Chain-01 under the posture-gated evaluator'
CHAIN01_MODEL="$KEEP/policy/chain01_model_best.pt"
CHAIN01_ENV="$KEEP/policy/chain01_env.yaml"
mkdir -p "$KEEP/policy"
if [[ ! -s "$CHAIN01_MODEL" || ! -s "$CHAIN01_ENV" ]]; then
  cp -a "$CHAIN01_ROOT/exported/model_best.pt" "$CHAIN01_MODEL"
  cp -a "$CHAIN01_ROOT/exported/env.yaml" "$CHAIN01_ENV"
fi
run_full_suite chain01 "$CHAIN01_ROOT" "$CHAIN01_MODEL" "$CHAIN01_ENV" "$CHAIN01_EXPECTED_SHA"
...
for entry in "${SCENARIO_CASE[@]}"; do
  run_video chain01 "$CHAIN01_ROOT" "$CHAIN01_MODEL" "$CHAIN01_ENV" \
    "${entry%%:*}" "${entry##*:}" 101
done
[[ "$(find "$KEEP/evaluation/chain01/videos" -name '*.mp4' | wc -l)" == 7 ]] || {
  echo '[FAIL] expected 7 videos for chain01'; exit 8;
}

echo '[PHASE 3/3] one-file result packaging'
printf 'RUNNER_RC=0\nCOMPLETED_AT=%s\nWORK_ID=G-A023\nRUN_ID=eval_260905-Go2_chain01_baseline\nTRAINING=none\nCHAIN01_MODEL_SHA=%s\nTELEMETRY_CHAIN01=69\nVIDEOS_CHAIN01=7\nEVALUATOR=posture_gate_v2\nVIDEO_STATUS=VIDEO_UNKNOWN\nOFFICIAL_RESULT=OFFICIAL_RESULT_UNMEASURED\n' \
  "$(date -Is)" "$CHAIN01_EXPECTED_SHA" >"$KEEP/RUNNER_STATUS.txt"
package_result FULL
trap - EXIT
echo '[DONE] GO2_CHAIN01_BASELINE_RESULT_READY'
