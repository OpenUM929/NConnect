"""IsaacLab 공식 RSL-RL play.py 위임 + quadruped task 등록.

══════════════════════════════════════════════════════════════════════
결과물 폴더: training/quadruped/exported/  (한 곳에 모두)
  ★ model_best.pt  : 예선 채점용 (train.py 가 만듦)
  ★ env.yaml       : 학습 환경 dump
  ★ policy.pt      : 본선 매치용 (play.py 가 export)
══════════════════════════════════════════════════════════════════════

구현: subprocess 분리 (IsaacLab play.py 가 os._exit() 호출해도 wrapper 살아남음).

사용:
    isaaclab.bat -p training\\quadruped\\play.py --task Quadruped-v0 --num_envs 4
    isaaclab.sh -p play.py --task Quadruped-v0 --num_envs 1 --headless --video --push   # 밀침 테스트
"""
...
import go2_task  # noqa: F401 — gym.register (가벼움: gymnasium + 문자열 등록만)

EXPORT_DIR    = PROJECT_DIR / "exported"
LOGS_ROOT     = PROJECT_DIR / "logs" / "rsl_rl" / "quadruped"


def _find_isaaclab_play() -> str:
    """IsaacLab rsl_rl play.py 경로 — OS/설치위치 무관 자동 탐지."""
...
def _runpy_isaaclab_play() -> None:
    """go2_task 등록 후 IsaacLab play.py 를 in-process runpy (자식에서 호출)."""
    import runpy
...
def run_play() -> int:
    """부모: play 를 python.sh 자식으로 실행 → 자식 강제종료해도 부모 생존 →
    __main__ 의 finally(policy.pt export)가 실행됨. python.sh 없으면 in-process 폴백."""
...
def _guard_gpu_free(need_mb: int = 4000) -> None:
    ...
    print("        play.py --task Quadruped-v0 --num_envs 16 --headless --video \\")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 어떤 상황에서도 (Ctrl+C, 에러, 정상 종료) policy.pt 동기화 시도.
        # IsaacLab 가 이미 logs/<run>/exported/policy.pt 만들었으면 그걸 root 로.
