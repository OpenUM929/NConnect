...
- [이번 테스트] **G-A027** — 동결 G-A017 후보와 Pilot-01을 재학습 없이 각각 69 case-run으로 같은 날 같은 evaluator에 올려 재고, 시나리오별 영상 7건을 남긴다.
- [흐름] 채점기 수리 완료(engine 1.5.4, 4차 재감사 동의) → **G-A027 측정 실행·회수** → 회수 검증 → Q1 스크리닝 판정 → 단일변수 재개 → H1·Go2 각 라운드 제출.
- [수집과 승급은 다른 질문이다] **수집**은 고정 ZIP으로 증거를 얻는 일이고, **승급**은 그 증거로 제출 후보를 정하는 일이다. 수집 조건(§8-c)을 통과했다는 것은 **수집에 착수해도 된다는 뜻일 뿐**이며, 측정이 유효한지는 회수 뒤 §8-d 검증으로 따로 판정한다. 승급은 모든 case의 생존·추종 인자를 각각 보고 판정한다(4차 감사 C6).
- [지금 할 일] 아래 「Go2 4족 — G-A027」 절의 업로드 1회(파일 1개) + 실행 1줄. **잔여 GPU는 시작 전 대시보드에서 실측한다 `[미측정]`** — 15시간(G-F192)은 전달받은 계획 입력일 뿐 현재 잔량이 아니다. G-A027 추정 1h45m도 상한이 아니므로 시계는 사람이 본다.
...
## Go2 4족 — G-A027: A017 후보·Pilot-01 69 case 전량 실측 (학습 없음, engine 1.5.4, 260908)

**상태: 업로드 대기 — 다음에 실행할 것. 이전 G-A025는 취소됐다(아래).**

**이름 읽는 법 — 이 회차 번호는 G-A027이다.** 파일·디렉터리·tmux 세션에 붙은 `a017`은
회차 번호가 아니라 **재는 대상 정책의 이름**이다. G-A017에서 나온 후보
(`track_lin_vel_xy_exp` 1.2→1.4)와 동결 Pilot-01, 두 정책을 나란히 재기 때문에 두 arm의
이름이 `a017`과 `pilot`이고 그것이 회수물 디렉터리 이름이 된다. 러너 자신도 결과에
`WORK_ID=G-A027`을 적는다(`server_run_go2_a017_full_suite.sh:456`). 번호가 뒤로 돌아간
것이 아니라 **파일 이름이 회차 자리에 정책 이름을 놓아 그렇게 읽히는 것**이며, 이 결함은
G-D175로 기록했다.

**이 작업은 학습을 하지 않는다.** 이미 동결돼 디스크에 있는 두 정책을 재생만 한다.
reward 파일도 건드리지 않으므로 제출 후보와 학습 아티팩트는 어느 것도 바뀌지 않는다.

### 왜 이 작업인가

engine 1.5.0~1.5.4 수리 후 저장된 tier-1 실행 11건을 전부 재판정한 결과, 비교가
차단되지 않는 것은 4건뿐이고(A015·A016·A017·A018) 그중 새 조기기각 게이트를 통과하는
것은 **G-A017 하나**다(`track_lin_vel_xy_exp` 1.2→1.4 단일 변수, tier-1 `+3.707916/70`,
허용 한계를 넘는 시나리오 곱 후퇴 0건). G6의 곱은 실제로 −0.006756 후퇴했다 —
"후퇴 없음"이라고 적었던 이전 문구는 틀렸고, 판정은 한계값 0.007 이상 어디서나
유지되므로 대입된 0.1에 기대지 않는다. 이 실행은 옛 생존-단독 조항으로 조기 사살돼
seed 202/303도 69-case suite도 받은 적이 없다. tier-1 절대값은 있으나(`50.199157/70` vs Pilot-01
`46.491241/70`, 7 case 척도) 69-case 최악값 척도의 점수가 없다 — **두 척도는 서로
비교할 수 없다.** 이 실행이 그 빠진 값을 두 정책 모두에 대해 같은 날 만든다.

### 왜 Pilot-01도 69건을 다시 재는가

G7 도메인 랜덤화 결함 수정으로 Pilot-01의 저장된 G7 3건은 사실 G3 숫자다. 저장된 69건
전부에서 높이 채널이 실측이고 `NCRC_EVAL_DR`이 환경에 설정된 적이 없음을 확인했으므로
"G7 3건만 갱신하고 66건 재사용"도 논증으로는 성립한다(G-F180). 그러나 그건 다른 날
다른 엔진 빌드로 잰 숫자에 대한 **논증이지 측정이 아니다.** 이 캠페인은 이미 그런 논증에
몇 주를 잃었다. 30분을 더 써서 양 arm을 같은 날 같은 evaluator 바이너리에 올린다(G-D123).

### 1. 로컬 준비 (완료)

`workspace\training\quadruped\upload\G-A027\current\`

### 2. 업로드 — 파일 1개

`go2_a017_full_suite.zip` — **G-A027 회차의 패키지다**(파일명의 `a017`은 재는 정책 이름).
SHA256 `e7749d0f6adc4abb2b32fca9393523b8059b8954cec068ba2c7c2930af0a7d15`

러너·evaluator·registry·두 정책 아티팩트가 모두 zip 안에 있다. engine zip과 spec json을
따로 올리지 않는다.

### 3. 실행 한 줄

```bash
cd /workspace && echo 'e7749d0f6adc4abb2b32fca9393523b8059b8954cec068ba2c7c2930af0a7d15  go2_a017_full_suite.zip' | sha256sum -c - && unzip -oq go2_a017_full_suite.zip && bash /workspace/go2_a017_full_suite/server_run_go2_a017_full_suite.sh
```

tmux 안에서 시작하고 즉시 리턴한다.

러너는 case마다 요약 JSON의 값을 검사하므로 서버에 `python3`가 필요하다. 없으면 실행
도중이 아니라 **시작 시점에** 멈추고 그 사실을 말한다.

### 4. 진행 확인

```bash
tmux attach -t go2_a017_full_suite
```

4단계: ① A017 69 case-run → ② Pilot-01 69 case-run → ③ 시나리오별 영상 7건 → ④ 패키징.
69는 23 case × seed 101/202/303의 **합계**다(207건이 아니다).

**예상 약 1시간 45분.** 이것은 과거 실측 wall time에서 나온 추정이며 **상한이 아니다** —
스크립트에는 시간 초과 시 중단하는 코드가 없다. 시계는 사람이 본다.

### 5. 완료 확인

`[DONE] GO2_A017_FULL_SUITE_RESULT_READY`
...
기본 재실행은 이전 결과를 지우지 않고 **거부한다**. 회수 전에 다시 돌리면 결과를 잃던
동작을 막은 것이다. 중단된 실행을 이어가려면 `GO2_RESUME=1`, 의도적으로 버리려면
`GO2_DISCARD_PREVIOUS=1`을 명시한다. resume은 파일 존재가 아니라 정책·case·seed·argv·
step 수·evaluator SHA 지문 일치로 판정하므로, 조건이 달라진 case는 다시 돈다.

### 8. 서버 종료 관문

DONE 표시만으로 종료하지 않는다. 두 파일을 먼저 받고, 로컬 검증에서 outer SHA · ZIP CRC ·
내부 manifest · `RUNNER_RC` · 정책 2종 SHA · 각 69건 telemetry · 영상 7건 · `EVALUATOR_SHA` ·
`TELEMETRY_SCHEMA=6`·`ENGINE_VERSION=1.5.4`가 확인된 뒤에만 종료 판단을 보고한다.

각 case 요약에는 `posture_coverage`·`posture_min_env_coverage`와
`posture_fall_verdict_ambiguous`가 들어 있다. 러너는 **case가 끝난 뒤** 그 요약을
검사하고, 미달이거나 모호하면 그 case의 생존값을 채택하지 않고 다음 case를 중지한다.
이상 프레임이 나온 그 순간에 play를 죽이지는 않으므로 해당 case의 실행 비용은
이미 쓴 것이다. 회수물에 69×2건이 모두 있다는 것은
전 env를 끝까지 관측했다는 뜻이자 **관측이 빠진 프레임이 낙상 판정을 바꿀 수
없었다**는 뜻이다. 커버리지만으로는 부족하다 — 0.99에서도 가장 나쁜 env가 1%를
놓칠 수 있고, 그 한 프레임이 0.8초짜리 주저앉음을 지울 수 있었다(2차 감사 C3).
관측이 있었는지와 그 관측이 수인지는 다른 문제다. `nonfinite_row_count`가 0이 아니면
그 case는 생존값을 내지 않고 러너가 실패시킨다 — 무한대는 모든 문턱을 통과하므로,
물리가 터진 스텝이 완벽하게 서 있는 로봇으로 읽혔다(3차 감사 C5). 이 계수는
**위치 3·속도 3·명령 3, 아홉 입력에 한정**한다. 0이라는 것은 그 아홉 개가 수였다는
뜻이지 모든 센서와 파생값이 온전하다는 뜻이 아니다 — 나머지는 회수 후 검증한다.

**한 case가 실패해도 이미 측정한 case가 무효가 되지는 않는다.** 러너는 그 자리에서
종료하므로 작업이 미완료가 될 뿐이다. 중단되면 그때까지의 결과를 먼저 회수한다.

### 8-b. 제출 시 백업 한 번 (GPU 0, G-D132)

제출 정책을 확정할 때 `NO_AUTO_SUBMIT`을 **해제한 상태로** finalize를 한 번 돌려 대회 서버에
`model_best.pt`·`env.yaml`·`report.html` 백업을 내보낸다. 학습이 필요 없고 GPU를 쓰지 않는다.
러너는 이미 기본 해제이며 계약 테스트 [10]이 이를 강제한다. 이것은 **공식 제출이 아니라**
대회 코드가 제공하는 편의 백업이며, 공식 제출은 팀 대시보드에서 직접 업로드한다.

주장의 범위를 좁혀 적는다(G-D134, 2차 감사 §4.2 수용). 배포 코드로 확인되는 것은
**이 스위치를 쓴 것만으로 위반이라고 볼 로컬 근거가 없다**는 것까지다. 운영진이
실제로 자격을 어떻게 판단하는지는 미확인이며, 배포 코드가 그것을 확정해 주지 않는다.
또 백업 ZIP의 내용(`model_best.pt`·`env.yaml`·선택적 `report.html`)은 수동 제출물과
같지 않고, 지금 백업을 한 번 돌리는 것은 **과거 학습 시점의 서버측 이력을 복구하는
것이 아니다**. 그리고 이 호출은 외부 POST를 포함하므로 GPU를 쓰지 않는다는 것이
부작용이 없다는 뜻은 아니다 — 정책 파일을 먼저 보존한 뒤 한 번만 돌린다.

### 8-c. 감사자가 요구한 수집 조건 (G-D143)

**잔여 GPU는 허가를 받는 조건이 아니다. 회수 여유는 조건이다 (G-D172, 8차 감사 §4)** —
15시간은 전달받은 계획 입력일 뿐 현재 잔량이 아니므로 그 숫자를 근거로 쓰지 않고,
잔량을 누구에게 보고하고 재승인을 기다리는 절차도 없다. 이 러너는 **case 단위로 이어
돌릴 수 있고**, 이어붙이기는 case 지문(모델·env·계측기 해시, 시나리오, seed, step 수,
DR·밀침·자세 임계값, 원시 명령)이 그대로이고 `EVAL_RC=0`인 case만 건너뛴다. 낡은
조건에서 잰 결과는 재활용되지 않는다.

그러나 **이어 돌릴 수 있다는 것은 만료 뒤에도 파일을 꺼낼 수 있다는 증거가 아니다.**
서버는 접속마다 초기화되고 회수하지 못한 `_keep`은 그대로 사라진다(아래 문단). 그래서
절차는 "잔량이 얼마든 시작하고 끊기면 이어 붙인다"가 아니라 **접속한 자리에서 잔량과
종료 조건을 보고, 회수에 필요한 여유를 남긴 채 멈춘다**이다. 여유가 빠듯하면 끝까지
밀지 말고 중간에 회수한 뒤 다음 세션에서 이어 간다. 이 판단은 접속한 사람이 그 자리에서
한다.

끊겼거나 멈추기로 했을 때 해야 하는 일은 하나다 — 그 자리에서 `_keep`을 통째로
내려받는다. 서버는 접속마다 초기화되므로 회수하지 않고 다음 세션을 열면 이어붙일 대상
자체가 사라지고 처음부터 다시 돈다. 다음 세션에서는 내려받은 `_keep`을 같은 경로로
되올린 뒤 `GO2_RESUME=1`로 이어 간다.

예산이 모자랄 때 **먼저 포기하는 것은 3단계 영상**이다. 1·2단계는 두 정책을 같은 날
같은 엔진·같은 계측기로 재는 것이 이 측정의 존재 이유이므로 쪼갤 수 없고, 4단계
묶기는 비용이 없다. 다만 영상을 생략하면 정량 측정은 성립해도 **행동 검증이 끝난 것은
아니다** — 판정은 `VIDEO_UNKNOWN`으로 남고 후보 승급은 보류되며, 생략 사유와 재현에
필요한 자료를 보존해 영상 회수를 남은 작업으로 적는다(8차 감사 §4). 아래 "종료 후"의
영상 7건 회수 요구는 취소되지 않는다.

**승인 env 해시는 이미 정해져 있다 (GPU 0, G-D165)** — 8-d가 요구하는 두 값은 서버에서
얻는 것이 아니라 **올릴 ZIP 안에 이미 들어 있다**. 회수물이 스스로 적어 온 값을 기대값으로
쓰면 자기가 자기를 증명하는 꼴이지만, 고정 ZIP은 감사자가 해시를 이미 들고 있는 물건이므로
그 안에서 뽑은 값은 순환이 아니다. 서버에 올리기 전에 로컬에서 한 번 뽑아 두면 되고,
값이 바뀌지 않는 한 다시 뽑을 필요도 없다.

```
python -B - <<'EOF'
import hashlib, zipfile
z = zipfile.ZipFile('workspace/training/quadruped/go2_a017_full_suite.zip')
for label in ('a017', 'pilot'):
    name = 'go2_a017_full_suite/%s/exported/env.yaml' % label
    print(label, hashlib.sha256(z.read(name)).hexdigest())
EOF
```

| arm | 승인 `env.yaml` SHA256 |
|---|---|
| `a017` | `41050c084cd05e7646ce2cb4ac34e06a6870fb7a65b4f767c5714611b9a801ff` |
| `pilot` | `f5550641c82aeb0a98892b8c74d61d6234d527733061fa3476338bf55b26975d` |

러너는 이 파일을 `_keep/policy/`로 복사해 쓰고 그 해시를 `identity.json`에 적는다. 이전
세션의 사본이 `_keep`에 남아 있으면 러너가 그것을 그대로 재사용하므로, 이 대조는 낡은
env로 잰 회수물을 잡아내는 검사이기도 하다.

**실행** — 위 고정 ZIP 해시로만 올린다. 이전 결과가 서버에 남아 있으면 먼저 회수한다.
discard 옵션으로 우회하지 않는다.

**첫 10분** — case가 실제로 진행되는지, `survival_proxy_source`·커버리지·모호성·
`nonfinite_row_count`가 기대대로 찍히는지, 로그와 저장 경로가 맞는지 본다.

**2시간 지점** — 러너 추정(약 1시간 45분)은 상한이 아니다. 자동 종료도 없다.
이 시점에 예산·속도·회수 여유를 다시 판단한다. 중단이 필요하면 현재 결과 보존과
회수가 우선이다.

**종료 후** — 두 arm 각각 69건 요약, 원시 CSV·metadata·지문·로그·manifest, 영상 7건을
회수한다. PARTIAL 생성이 실패할 수 있다는 전제로 `_keep` 원시 파일도 함께 가져온다.

**로컬 판정 전 (GPU 0)** — 유한성 검사, 두 arm의 source·DR·명령·seed·길이 대조.
**invalid case를 빼고 전체 점수를 억지로 완성하지 않는다** — 영향 case는 격리하고
전체 측정은 미완료로 표시한다.

**승급 전 (GPU 0)** — 모든 case의 생존·추종 인자를 각각 보고, 절대 미달과 상대 개선을
분리해 적는다. worst-product case 하나만 보는 자동 승급 결과는 채택하지 않는다.

### 8-d. 회수 검증 — 점수를 읽기 전에 한 번 (GPU 0, G-D147)

회수물을 받은 그 자리에서, 어떤 수치도 읽기 전에 돌린다.

```
python -B tools/verify_go2_a027_harvest.py --harvest workspace/_keep/go2_a017_full_suite \n  --expect-env-sha a017=41050c084cd05e7646ce2cb4ac34e06a6870fb7a65b4f767c5714611b9a801ff --expect-env-sha pilot=f5550641c82aeb0a98892b8c74d61d6234d527733061fa3476338bf55b26975d \n  --out workspace/_keep/go2_a017_full_suite/harvest_verification.json
# EVAL_STEPS를 런너 기본값(1000)과 다르게 쓴 경우에만 --expect-steps <실제값> 을 덧붙인다.
```

계측기가 자기 출력을 검사하고 러너가 자기가 쓴 요약을 검사한 것은 **둘 다 만든 쪽의
말**이다. 이 도구는 받는 쪽의 검사이고, `summary.json`을 `steps.csv`에 대한 **주장으로만**
취급한다. 범위 검사는 재계산이 아니므로(5차 감사), 채점이 실제로 소비하는 수치 —
생존, 추종 RMSE, 밀침 뒤 RMSE, 기립 회복률, 진행량, 커버리지 — 를 계측기 자신의 정의로
원시 행에서 다시 계산해 요약과 대조한다. 그 밖에 확인하는 것: 69건이 registry가 지정한
그 69건인지, (step, env) 격자가 빠짐도 중복도 없이 완전한지, `upright` 열이 계측기가 쓸 수
있는 세 값 안에 있고 그 값이 같은 행의 자세 채널과 어긋나지 않는지, case마다 **그 시나리오가
채점받는** 수치(밀침이면 밀침 뒤 RMSE와 기립 회복률, 계단이면 진행량)를 갖고 있는지,
`STATUS.txt`를 key=value로 파싱했을 때 `EVAL_RC`가 정확히 `0`인지, `metadata.json`이 요약과
같은 실행을 가리키는지, 그리고 두 arm이 같은 자로 재였을 뿐 아니라 **그 자가 승인된
실행 계획의 자인지** — 모델·계측기·registry 해시와 env/step 수를 런너 스크립트에서
읽어와 대조한다.

여기에 6차 감사가 지적한 세 가지가 더해졌다(G-D158). 첫째, **시각을 시각으로 검사한다** —
계측기는 `time_s`를 자기 step 번호와 step 간격의 곱으로 적는데, 받는 쪽은 그 값을 읽기만
하고 대조하지 않았다. 시각은 낙상 유예 구간과 밀침 이후 구간을 자르는 기준이므로, 넘어진
행들의 시각만 유예 구간 안으로 옮겨 적으면 낙상이 통째로 사라졌다. 이제 시각이 step
번호와 맞지 않거나, 숫자가 아니거나, 행 순서가 step 순서를 거스르면 거절한다. 둘째,
**상대 높이를 그 재료로 되돌려 검사한다** — 상대 높이는 몸통 높이에서 지면 높이를 뺀
값인데 그 뺄셈을 확인하지 않아, 지면을 100 m로 적어도 로봇은 서 있는 것으로 읽혔다.
다만 지면을 못 읽은 정상적인 결측(지면·높이·자세가 모두 빈 칸)은 그대로 결측으로 남긴다.
셋째, **env 해시는 있는지가 아니라 무엇인지를 본다** — 이전에는 값이 비어 있지 않기만
하면 통과였다. 이제 sha256 모양이 아니면 거절하고, 승인된 env 해시는 `--expect-env-sha
a017=<sha> --expect-env-sha pilot=<sha>`로 **바깥에서** 넣어준다. 회수물 자신이 적어 온
값을 기대값으로 쓰면 자기가 자기를 증명하는 셈이라서다. 승인 해시를 못 넣으면 판정은
`INTERNAL_GATE_INCONCLUSIVE`에서 더 올라가지 않는다.

`INTERNAL_GATE_PASS`가 아니면 **어떤 점수도 읽지 않는다.** 실행 계획을 읽지 못하면
자료가 스스로 앞뒤가 맞더라도 `INTERNAL_GATE_INCONCLUSIVE`이지 PASS가 아니다. 실패한 case는 이름이
찍히며, 그 case를 빼고 68건으로 점수를 완성하지 않는다 — 이 도구는 정확히 그 동작을
막으려고 있다. 계약 검사는 `tools/test_go2_harvest_verifier_contract.py`이며, 손상시킨
합성 회수물에서 각 검사가 실제로 거절하는지를 확인한다.

### 9. 결과 판독 — 실행 전에 고정 (G-D124)

**Q1 스크리닝 — 이 다이얼을 유지할 것인가.** 두 69-case 성적표를 비교한다. 가중 총점이
높고 시나리오 곱 후퇴가 게이트를 넘지 않으면 다이얼 유지, A017이 이후 단일변수 실험의
동결 기준선이 된다. 이 실행이 답할 수 있는 것은 여기까지다.

**Q2 제출 — 둘 중 하나를 대회에 낼 수 있는가.** `go2_self_eval_registry.json`의
`score.internal_gates`만으로 판정한다(시나리오별 생존 0.95 **및** 추종 0.70, 가중 proxy
0.70, 자체평가 총점 70, seed 3종). tier-1 실측에서 **두 정책 모두 `INTERNAL_GATE_FAIL`**
이므로 예상 결과는 **상대 개선 + 절대 불합격**이다. 미달 인수는 시나리오마다 다르다 —
G3는 생존·추종 둘 다, **G4·G5는 생존만**(추종은 이미 0.70을 넘겼다), G7은 추종만이다
(G-F184; 종전 "G3·G4·G5·G7 추종 미달" 서술은 틀렸다). 다음 단일변수는 이 표에서
약한 인수를 읽어 고르므로 이 구분이 중요하다.
Q1을 이기고 Q2를 지는 정책은 새 스크리닝 기준선이지 제출물이 아니다.

평가 seed 3개는 한 정책의 재생 3회이지 독립 학습 seed 3개가 아니다 — 측정 잡음의 범위를
주는 것이지 학습 변동성을 주지 않는다. 후보가 기준선 이하로 나와도 닫히는 것은 **값 1.4**
이지 방향 전체가 아니다.

이 실행은 다음 reward 변수를 스스로 고르지 않는다. 공식 evaluator·공식 점수는 여전히
...
**상태: 취소 — 실행하지 않는다(260908, G-D115/G-D123).** 이 실험은 Default-01을 기준선으로 삼는데, 재판정 결과 Default-01은 보행하지 않는 정책이다(`POLICY_DOES_NOT_LOCOMOTE` 51/69). engine 1.5.1 이후는 보행하지 않는 기준선 위의 실험 사양을 검증 단계에서 거부하므로 이 spec은 더 이상 실행 가능한 상태가 아니다. 다음 실행은 위 **G-A027**이다. 아래 내용은 이력으로 남긴다.
