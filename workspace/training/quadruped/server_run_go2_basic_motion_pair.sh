#!/usr/bin/env bash
# One upload -> one command -> one result ZIP, for the basic-motion pair G-A031 + G-A032.
#
# The pair ZIP carries both arm packages byte for byte as published (upload/G-A031,
# upload/G-A032) and runs them one after the other on one GPU:
#   1  the target stage of each arm, through the arm's own staged runner (--inner):
#      training, catastrophe gate, the 9 target cases, the sentinel, 5 videos
...
#   4  one result ZIP: both arm result ZIPs, the gate records, PAIR_STATUS.txt
...
PAIR_ROOT=${PAIR_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}
[[ -s "$PAIR_ROOT/PAIR_SHA256SUMS.txt" && -s "$PAIR_ROOT/pair_config.env" ]] || {
  echo "[FAIL] pair package must be extracted at $PAIR_ROOT"; exit 2;
}
(cd "$PAIR_ROOT" && sha256sum -c --quiet PAIR_SHA256SUMS.txt) || {
  echo '[FAIL] pair package checksum mismatch'; exit 2;
}
# shellcheck source=/dev/null
source "$PAIR_ROOT/pair_config.env"

WORKSPACE=/workspace
PAIR_KEEP="$WORKSPACE/_keep/$PAIR_KEEP_NAME"
PAIR_RESULT="$WORKSPACE/_keep/$PAIR_RESULT_ZIP"
ARM_RUNNER=server_run_go2_candidate_staged.sh
GATE="$PAIR_ROOT/go2_target_gate.py"
STORED="$PAIR_ROOT/stored_baseline"
RESUME=${GO2_RESUME:-0}
...
full_complete() {
  [[ "$(status_of "$1" RUNNER_RC)" == 0 && "$(status_of "$1" STAGE)" == full &&
     "$(status_of "$1" DECISION)" == SUITE_COMPLETE ]]
}
baseline_remeasured() {
  [[ -d "$(arm_keep "$1")/evaluation/$(arm_var "$1" BASELINE_LABEL)/cases" ]]
}

# The arm runner's own preflight, for both arms before anything starts, because
# the pair calls each arm runner with --inner, which skips it.
preflight_arm() (
  root=$1 work=$2
...
previous_results() {
  local root path
  for path in "$PAIR_KEEP" "$PAIR_RESULT"; do
    [[ ! -e "$path" ]] || echo "$path"
  done
...
if [[ "${1:-}" != "--inner" ]]; then
  ...
  [[ -s "$GATE" && -d "$STORED" ]] || { echo "[FAIL] gate or stored baseline missing in $PAIR_ROOT"; exit 2; }
  ...
  for i in "${!ARM_WORK[@]}"; do
    zip="$PAIR_ROOT/arms/${ARM_ZIP[$i]}"
    [[ "$(sha256sum "$zip" | awk '{print $1}')" == "${ARM_ZIP_SHA[$i]}" ]] || {
      echo "[FAIL] ${ARM_WORK[$i]} package ${ARM_ZIP[$i]} is not the published one"; exit 2;
    }
  ...
  for name in "$PAIR_TMUX" $(for root in "${ARM_ROOT[@]}"; do arm_var "$root" TMUX_NAME; echo; done); do
  ...
  printf -v INNER_COMMAND "cd %q && PAIR_ROOT=%q GO2_RESUME=%q ISAACLAB_SH=%q bash server_run_go2_basic_motion_pair.sh --inner" \
    "$PAIR_ROOT" "$PAIR_ROOT" "$RESUME" "$ISAACLAB_SH"
  tmux new-session -d -s "$PAIR_TMUX" "$INNER_COMMAND"
  echo "[STARTED] $PAIR_TMUX pair=$PAIR_ID arms=${ARM_WORK[*]} resume=$RESUME"
  echo "[MONITOR] tmux attach -t $PAIR_TMUX"
  # An estimate from prior wall time, not a cap: nothing here stops the run.
  echo "[ESTIMATE] target stage per arm ~1h25m -> both ~2h50m; each arm the gate passes adds its full stage ~55m"
  echo "           both pass ~4h40m; a REMEASURE_BASELINE retry adds ~20m to that arm's target stage"
  echo "           and ~45m to its full stage"
  echo "[NO_DEADLINE] the script does not enforce a time limit; watch the clock yourself"
  echo "[DOWNLOAD_WHEN_DONE] $PAIR_RESULT"
  echo "[DONE_MARKER] $PAIR_DONE_MARKER"
  exit 0
fi

mkdir -p "$PAIR_KEEP/gate" "$PAIR_KEEP/arm_results"
LOG="$PAIR_KEEP/PAIR_LOG.txt"
log() { printf '[%s] %s\n' "$(date -Is)" "$*" | tee -a "$LOG"; }
...
write_pair_status() {
  ...
  {
    printf 'PAIR_RC=%s\nRESULT_STATE=%s\nWRITTEN_AT=%s\nPAIR_ID=%s\nRELEASE_ID=%s\n' \
      "$rc" "$state" "$(date -Is)" "$PAIR_ID" "$PAIR_RELEASE_ID"
    for root in "${ARM_ROOT[@]}"; do
  ...
  } >"$PAIR_KEEP/PAIR_STATUS.txt"
}
package_pair() {
  local state=$1 rc=$2 root result
  for root in "${ARM_ROOT[@]}"; do
    result=$(arm_result "$root")
    [[ ! -s "$result" ]] || cp -a "$result" "$PAIR_KEEP/arm_results/"
    [[ ! -s "${result}.sha256" ]] || cp -a "${result}.sha256" "$PAIR_KEEP/arm_results/"
  done
  write_pair_status "$rc" "$state"
  (
    cd "$PAIR_KEEP"
  ...
  "$ISAACLAB_SH" -p "${ARM_ROOT[0]}/package_go2_result.py" "$PAIR_KEEP" "$PAIR_RESULT"
  sha256sum "$PAIR_RESULT" >"${PAIR_RESULT}.sha256"
  echo "[DOWNLOAD] $PAIR_RESULT"
  echo "[DOWNLOAD] ${PAIR_RESULT}.sha256"
}
on_exit() {
  local rc=$?
  trap - EXIT
  if [[ "$rc" != 0 ]]; then
    log "[FAIL] pair runner rc=$rc; packaging what exists"
    package_pair PARTIAL "$rc" || true
  fi
}
trap on_exit EXIT
...
run_gate() {
  local root=$1 tag=$2 work rc=0
  work=$(arm_var "$root" WORK_ID)
  "${PY[@]}" "$GATE" --keep "$(arm_keep "$root")" --package "$root" --stored "$STORED" \
    --out "$PAIR_KEEP/gate/${work}_${tag}.json" >>"$LOG" 2>&1 || rc=$?
  case "$rc" in
    0) echo TARGET_PASS ;;
    10) echo FAIL ;;
    11) echo REMEASURE_BASELINE ;;
    *) echo UNDECIDED ;;
  esac
}
target_phase() {
  ...
  if ! target_complete "$keep"; then
    ...
    if ! run_stage "$root" target "$resume" "$remeasure"; then
      ...
      log "[ARM FAILED] $work target stage; the other arm goes on"
      return 0
    fi
  fi
  ...
  if [[ "$verdict" == REMEASURE_BASELINE && "$remeasure" == 0 ]]; then
    ...
    if ! run_stage "$root" target 1 1; then
      STATE[$work]=TARGET_RUNNER_FAILED
      log "[ARM FAILED] $work target stage with A017 remeasured; the other arm goes on"
      return 0
    fi
...
full_phase() {
  ...
  if baseline_remeasured "$root"; then remeasure=1; fi
  if run_stage "$root" full 1 "$remeasure" && full_complete "$keep"; then
    STATE[$work]=FULL_DONE
  else
...
log "[START] pair $PAIR_ID ($PAIR_RELEASE_ID) resume=$RESUME"
(cd "$PAIR_ROOT" && sha256sum -c --quiet PAIR_SHA256SUMS.txt)
...
log '[PHASE 3/3] one-file pair result'
for root in "${ARM_ROOT[@]}"; do
  work=$(arm_var "$root" WORK_ID)
  log "[RESULT] $work state=${STATE[$work]:-NOT_RUN} gate=${VERDICT[$work]:-NOT_READ}"
done
package_pair PAIR_COMPLETE 0
trap - EXIT
echo "$PAIR_DONE_MARKER"
