...
- candidate ??? `TRAIN_RC=0`?? ????.
- ?? 8? ? G2 `combined_yaw_left`, seed 101? ?? Isaac Sim startup? `XOpenDisplay` ???? segmentation fault? ????.
- v1 telemetry hook? ?? step? ???? `env.step()` ???? ????? ?? ????. ? ??? upstream Isaac Lab `play.py`? `env.close()`? `simulation_app.close()`? ???? ???.
- v1 runner? case ??? ????? ?? ? ???? ?? ??? `RUNNER_RC=5`? ????.

### 9-b. ?? ??

- **?? ?? ??? ?? ??:** telemetry? ?? ???? ??? case ?? ??? ??.
- **?? ??? ??:** Isaac Sim 5.1? startup segmentation fault ??? NVIDIA Kit ???? ????? ?? Python??? ? C++ ??? ????? ??? ? ??.
- ??? v2? upstream ?? ??? ????, ?? startup? ?? ??? ?? ??? ???? ??? bounded retry? ????.

### 9-c. v2 ?? ??

1. telemetry hook? ?? ???? ?? ??? ??.
2. ?? horizon ? `simulation_app.is_running()`? false? ?? upstream `env.close()`? `simulation_app.close()`? ????.
3. ? telemetry?video case? ?? 3?? ?????, ?? ???? PARTIAL? ???? ????.
4. ?? case fingerprint? candidate checkpoint? ?????? ????? ???.
5. active `launcher.log`? manifest? ZIP?? ???? packaging ??? `launcher.snapshot.log`? ????.
