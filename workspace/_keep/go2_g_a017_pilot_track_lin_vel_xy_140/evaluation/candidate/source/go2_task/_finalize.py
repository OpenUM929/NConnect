"""학습 종료 후 best model 선별 + training/quadruped/exported/ 에 출력.

출력 파일 (training/quadruped/exported/ 안 — 한 곳에 모음):
  - model_best.pt   : 예선 채점용 RSL-RL checkpoint
  - env.yaml        : 학습 환경 dump
  - policy.pt       : (play.py 실행 후) 본선 매치용 JIT exported

학생/운영자가 exported/ 한 폴더만 보면 됨.
"""
...
PROJECT_DIR = Path(__file__).resolve().parent.parent  # training/quadruped/
LOG_ROOT = PROJECT_DIR / "logs" / "rsl_rl" / "quadruped"
...
def find_best_model(run_dir: Path) -> tuple[Path, float | None, int | None]:
    ...
    models = sorted([m for m in run_dir.glob("model_*.pt") if _model_iter(m) >= 0],
                    key=_model_iter)
    if not models:
        raise FileNotFoundError(f"model_*.pt 없음: {run_dir}")
...
def _cleanup_intermediate_checkpoints(run_dir: Path, keep_iter: int | None = None) -> int:
    """run_dir 안의 model_*.pt 모두 삭제 (keep_iter 만 보존).
    tfevents/params/exported 는 보존 (tensorboard, 분석, resume 용).
    keep_iter 는 play.py 호환 — IsaacLab play.py 가 logs/ 에서 model 찾음.
    """
    deleted_count = 0
    deleted_bytes = 0
...
def finalize(run_dir: Path | None = None, cleanup: bool = True) -> dict:
    ...
    # 타임스탬프 — 실수 덮어쓰기 방지 백업
    import re
    ts = re.sub(r"[-_]|:", "", run_dir.name)
    if not ts.isdigit() or len(ts) < 8:
        from datetime import datetime
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")

    # ─── 예선 제출용 2개 (latest + timestamp 백업) ─────────────
    ...
    print(f"    > isaaclab.bat -p play.py --task Quadruped-v0 --num_envs 4")
...
#    ⚠️ h1_task/_finalize.py 와 동일 — 한쪽 수정 시 미러.
# ════════════════════════════════════════════════════════════════════
