#!/usr/bin/env bash
# Frozen Run06 model_9900 validation on three predeclared unseen seeds. No training.
set -euo pipefail
...
KEEP="/workspace/_keep/${RUN_ID}_independent_eval"
TMUX_NAME=run06_independent_eval
...
package_now() {
  ...
  local tarball="/workspace/_keep/${RUN_ID}_INDEPENDENT_EVAL_${label}.tar.gz"
  local tmp="${tarball}.tmp"
  tar -C /workspace/_keep -czf "$tmp" "${RUN_ID}_independent_eval"
...
if [[ "${1:-}" != "--inner" ]]; then
  ...
  tmux new-session -d -s "$TMUX_NAME" \
    "cd '$ROOT' && EVAL_RESUME='$RESUME' bash server_run06_independent_eval.sh --inner"
  echo "[STARTED] $TMUX_NAME"
  echo "[MONITOR] tmux attach -t $TMUX_NAME"
  echo "[RESULT] /workspace/_keep/${RUN_ID}_INDEPENDENT_EVAL_FULL.tar.gz"
  exit 0
fi
...
mkdir -p "$KEEP/seeds" "$KEEP/logs" "$KEEP/meta" exported
...
sha256sum play.py eval_telemetry.py fixed_eval_report.py independent_eval_report.py \
  humanoid_rewards.py h1_task/*.py >"$KEEP/meta/source.sha256"
...
printf 'seed\tscenario\tvx\tvy\twz\tpush_mps\tterrain\tsteps\tplay_rc\n' >"$KEEP/EVAL_STATUS.tsv"
...
run_case() {
  local seed=$1 scenario=$2 vx=$3 vy=$4 wz=$5 push=$6 terrain=$7 steps=$8
  shift 8
  local -a terrain_args=("$@")
  local seed_root="$KEEP/seeds/seed_${seed}"
  local out="$seed_root/cases/$scenario"
  mkdir -p "$seed_root/cases"
  if [[ "$RESUME" == 1 && -s "$out/STATUS.txt" ]] && grep -qx 'EVAL_RC=0' "$out/STATUS.txt"; then
    echo "[SKIP] seed=$seed scenario=$scenario already complete"
    return 0
  fi
  rm -rf -- "$out"
  mkdir -p "$out"
  ...
  {
    echo "seed=$seed scenario=$scenario terrain=$terrain steps=$steps"
    printf 'COMMAND: '; printf '%q ' "${cmd[@]}"; printf '\n'
  } | tee "$KEEP/logs/seed_${seed}_${scenario}.command.txt"
  set +e
  NCRC_EVAL_OUT="$out" NCRC_EVAL_STEPS="$steps" "${cmd[@]}" \
    2>&1 | tee "$KEEP/logs/seed_${seed}_${scenario}.log"
  local rc=${PIPESTATUS[0]}
  set -e
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$seed" "$scenario" "$vx" "$vy" "$wz" "$push" "$terrain" "$steps" "$rc" \
    >>"$KEEP/EVAL_STATUS.tsv"
  if [[ "$rc" != 0 || ! -s "$out/summary.json" || ! -s "$out/steps.csv" ]] || \
     ! grep -qx 'EVAL_RC=0' "$out/STATUS.txt"; then
    echo "[FAIL] seed=$seed scenario=$scenario rc=$rc or telemetry missing"
    exit 3
  fi
  echo "[OK] seed=$seed scenario=$scenario"
}

for seed in "${SEEDS[@]}"; do
  run_case "$seed" H1_stand   0.00  0.00  0.00 0 plane 1000 "${PLANE[@]}"
  run_case "$seed" H2_forward 0.75  0.00  0.00 0 plane 1000 "${PLANE[@]}"
  run_case "$seed" H3_left    0.00  0.30  0.00 0 plane 1000 "${PLANE[@]}"
  run_case "$seed" H3_right   0.00 -0.30  0.00 0 plane 1000 "${PLANE[@]}"
  run_case "$seed" H4_left    0.50  0.15  0.50 0 plane 1000 "${PLANE[@]}"
  run_case "$seed" H4_right   0.50 -0.15 -0.50 0 plane 1000 "${PLANE[@]}"
  run_case "$seed" H5_rough   0.50  0.00  0.00 0 rough 1000 "${ROUGH[@]}"
  run_case "$seed" H6_plus10  0.50  0.00  0.00 0 slope_inv 1000 "${SLOPE_UP[@]}"
  run_case "$seed" H6_minus10 0.50  0.00  0.00 0 slope 1000 "${SLOPE_DOWN[@]}"
  run_case "$seed" H7_push    0.00  0.00  0.00 0.5 plane 1000 "${PLANE[@]}"
done

/workspace/IsaacLab/isaaclab.sh -p independent_eval_report.py "$KEEP"
printf 'RUNNER_RC=0\nCOMPLETED_AT=%s\nMODEL_SHA=%s\nSEEDS=%s\n' \
  "$(date -Is)" "$MODEL_SHA" "${SEEDS[*]}" >"$KEEP/RUNNER_STATUS.txt"
package_now FULL
trap - EXIT
echo '[DONE] Run06 independent multi-seed evaluation complete'
