"""Contract for the G-A041 one-command campaign (G-A033 + ang_vel_xy_l2 -0.05 -> -0.04).  Local only.
...
What is new in this arm, and what these tests pin down:
  This is the relaxation side of the one dial whose strengthening was measured against the
  climbed-robot counter (90 -> 5 at G-A038).  The dial is one of the six names in the deployed
  REWARD_WEIGHTS list, so the candidate reward file changes a number rather than gaining a line, and
  -0.05 is the value Isaac Lab v2.3.1 writes in all three configs it ships - so the arm has no
  external anchor and the spec must carry a departure reason (test_12).  Two of its three target
  groups are its own predicted losses (test_9), and the 15 cm climb guard is left out because its
  floor is not positive (test_10).

    python -m unittest tools.test_go2_g_a041_campaign_contract
"""
...
class PackageTest(unittest.TestCase):
    ...
    def test_2_the_release_note_does_not_claim_a_history_it_does_not_have(self) -> None:
        """2026-09-20 결함: 첫 발행인데 '직전 판은 옛 판정 규칙으로 대체됐다'가 붙어 나왔다."""
        ...
        for work_id in ("G-A035", "G-A037", "G-A038", "G-A039"):
            self.assertIn("supersede_note", build.CAMPAIGNS[work_id],
                          f"{work_id} 의 이력 문장이 사라졌다")
    ...
    def test_3_campaign_runner_and_gate_are_what_ran_for_g_a033(self) -> None:
        ...
        # 2026-09-24 (defect C-32): 이 회차는 2026-09-22 에 러너가 고정됐다(C-14).  그 뒤로 팔의
        # 러너는 **작업본 파일이 아니라 자기가 돌았던 바이트**이므로, 작업본과의 동일성을 묻던
        # 옛 단정은 고정이 생긴 날부터 거짓이 됐다.  물어야 할 것은 발행 바이트와의 동일성이다.
    ...
    def test_4_exactly_one_reward_number_moves(self) -> None:
        ...
        self.assertEqual(reward_dict(reference)[TERM], SPEC["single_change"]["from"])
        ...
        for line in (f"SINGLE_CHANGE_NAME={TERM}", "SINGLE_CHANGE_FROM=-0.05", "SINGLE_CHANGE_TO=-0.04",
    ...
    def test_6_relaxing_raises_the_walk_margin_and_the_grid_says_so(self) -> None:
        ...
        relax, strengthen = probes[(TERM, "-0.04")], probes[(TERM, "-0.08")]
        self.assertEqual(relax["zone"], "WALK")
        self.assertGreater(float(relax["margin"]), float(relax["margin_from"]))
        self.assertLess(float(strengthen["margin"]), float(strengthen["margin_from"]))
        self.assertEqual(relax["range_status"], "OUT_OF_RANGE")
        # the walk-zone edge is on the strengthening side, so this arm moves away from it
        self.assertGreater(SPEC["single_change"]["to"], float(relax["edge_weight"]))
        self.assertIn(relax["margin"], SPEC["value_derivation"]["walk_margin"])
        self.assertEqual(base_data.walking_values(TERM), [SPEC["single_change"]["from"]],
                         "no walking run has ever trained another value for this term")

    def test_7_the_four_situation_cells_are_filled_and_the_signs_are_declared(self) -> None:
        situ = {(r["term"], r["to"]): r for r in mech.read("PROBE_SITUATIONS.csv")}[(TERM, "-0.04")]
        ...
        self.assertEqual(predictions["climb"]["direction"], "up")
        self.assertEqual(predictions["walk"]["direction"], "up")
        self.assertEqual(predictions["sway"]["direction"], "down")
        self.assertEqual(predictions["push"]["direction"], "down")
        # the predicted losses may not be quoted without the counterevidence rows that carry them
        self.assertTrue(SPEC["inference"]["contradicting"])
    ...
    def test_9_the_predicted_losses_are_measured_in_stage_one(self) -> None:
        """예측된 손실을 추정으로 남기지 않는다 — 표적 묶음에 넣어 1단계에서 잰다."""
        ...
        self.assertTrue(all(e.startswith("G6:") for e in groups["push"]))
    ...
    def test_10_every_climb_guard_can_actually_fire(self) -> None:
        ...
        self.assertTrue(str(SPEC["preregistered"]["climb_guard"].get("omitted", "")).strip(),
                        "빠진 묶음이 있으면 왜 뺐는지 사양이 말해야 한다")
    ...
    def test_11_the_isaac_lab_anchor_is_left_and_the_spec_says_why(self) -> None:
        ...
        anchors = {name: rewards[name][TERM] for name in ("base", "go2_rough", "go2_flat")}
        self.assertEqual(set(anchors.values()), {SPEC["single_change"]["from"]},
                         "this term is the one Isaac Lab keeps identical in all three configs")
        ...
        self.assertIn("추천 -0.3 ~ -0.02", deployed)
        self.assertEqual(reward_dict(deployed)[TERM], SPEC["single_change"]["from"])

    def test_12_the_earlier_releases_are_untouched(self) -> None:
        """이 회차가 앞 회차의 발행물을 건드리지 않았다.

        2026-09-20 결함 C-2: G-A035·G-A037·G-A039 의 발행 ZIP 은 **이 세션 이전부터** 현재 사양과
        어긋나 있다(사양을 발행 뒤에 고치고 ZIP 을 그대로 뒀다).  그래서 셋은 재빌드 대조를 통과할
        수 없다.  그 사실을 여기서 숨기지 않고 **래칫**으로 고정한다: 어긋난 집합이 정확히 그 셋일
        때만 통과하므로, C-2 를 고치면 이 테스트가 먼저 실패해 목록을 줄이라고 말한다.  G-A038 은
        같은 조건의 대조군이고 바이트까지 일치해야 한다.
        """
    ...
    def test_13_a_second_reward_change_is_refused(self) -> None:
...
class GateAndVerifierTest(unittest.TestCase):
    """The server gate and the verifier of record read a G-A041 harvest the same way."""
    ...
    def copy_cases(source: Path, out: Path, entries: list[str], model_sha: str,
        ...
        # 2026-09-24 (defect C-31): 진짜 수확물의 identity.json 은 `env_sha256` 을 쓴다.  이 가짜
        # 수확물만 빼먹어서 판독기가 INCONCLUSIVE 를 냈고, 아래 두 검사가 진짜 단정에 닿지 못했다.
    ...
    def harvest(self, name: str) -> Path:
        ...
        model = b"synthetic G-A041 model"
        model_sha = sha(model)
    ...
    def test_3_a_wrong_trained_weight_is_caught(self) -> None:
        """The whole arm rests on the trained env carrying -0.04; the stored baseline carries -0.05."""
