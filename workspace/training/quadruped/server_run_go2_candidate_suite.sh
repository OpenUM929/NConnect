...
#
# Candidate-suite runner (first used by G-A030).  One single-variable training
# run, then the full 69-case posture-gated G1-G7 suite on the trained candidate.
#
# Generalised from server_run_go2_a017_full_suite.sh (G-A027), which measured
# two frozen policies and trained nothing.  set_case, run_eval_case, run_video,
# stage_policy, the posture check and packaging are carried over from it
# unchanged; the training phase and its report recovery are carried over from
# server_run_go2_tuning_engine_v1.sh.  What is new:
#   - one scored arm, the trained candidate, instead of two frozen ones
#   - the baseline arm is an earlier full-suite measurement.  It is reused only
#     because this package ships the evaluator and registry that measured it
#     (checked below) and only if five of its cases, measured again in this
#     run, agree with the stored ones (checked at harvest).
#     GO2_REMEASURE_BASELINE=1 measures all 69 baseline cases here instead, on
#     the same evaluator binary and server as the candidate.
#   - a catastrophe stop: a non-finite training loss, or a candidate that
#     stands still on the catastrophe case, ends the run before the suite
#   - the run identity (work id, dial, cases, videos, paths) is run_config.env
#
# Phases
#   1  single-variable training; report.html preserved before any evaluation
#   2  catastrophe gate
#   3  candidate 69-case suite, seeds 101/202/303
#   4  baseline arm: sentinel cases, or all 69 with GO2_REMEASURE_BASELINE=1
#   5  videos
#   6  one-file packaging
set -euo pipefail

PACKAGE_ROOT=${PACKAGE_ROOT:-/workspace/go2_g_a030}
...
if [[ "${1:-}" != "--inner" ]]; then
  ...
  printf -v INNER_COMMAND "cd %q && PACKAGE_ROOT=%q GO2_RESUME=%q GO2_REMEASURE_BASELINE=%q ISAACLAB_SH=%q bash server_run_go2_candidate_suite.sh --inner" \
    "$PACKAGE_ROOT" "$PACKAGE_ROOT" "$RESUME" "$REMEASURE" "$ISAACLAB_SH"
  tmux new-session -d -s "$TMUX_NAME" "$INNER_COMMAND"
  echo "[STARTED] $TMUX_NAME work=$WORK_ID resume=$RESUME remeasure_baseline=$REMEASURE"
  echo "[MONITOR] tmux attach -t $TMUX_NAME"
  # An estimate from prior wall time, not a cap: nothing here stops the run.
  echo "[ESTIMATE] training ~57m + 69 candidate cases ~50m + ${#SENTINEL_CASES[@]} sentinel cases ~4m"
  echo "           + $(( ${#CANDIDATE_VIDEOS[@]} + ${#BASELINE_VIDEOS[@]} )) videos ~9m + packaging ~10m ~= 2h10m (2h35m with 20% margin)"
  echo "           GO2_REMEASURE_BASELINE=1 replaces the sentinel with 69 baseline cases: +~50m"
...
echo '[PHASE 3/6] candidate full 69-case suite'
...
finish SUITE_COMPLETE "$BASELINE_ARM"
