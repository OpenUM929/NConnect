...
## 13. H1-CAL-20260910 — 실제 공식 결과와 조건부 비교 (2026-09-11 기록)
- 사용자 전사 OFFICIAL_RESULT는57.45/70. 자체 Run06은65.7316336/70; 동일 제출본 조건에서8.2816336점 차이. 공식 제출 policy 대응은 아직 미확인이다.
- 기존/변경/공식 세 열을 보존한다:65.73163 /57.45 /57.45. 보정값이 실제값과 같음은 같은 표본으로 계수를 만들었기 때문이지 예측 정확도 검증이 아니다.
- §12-b가 H5/H6 POSTURE_UNMEASURED라고 밝힌 이상 §12-c~e의 전 시나리오 생존확정·전범위65.73 확보·잔여손실 전부tracking 표현은 현행 검증결론으로 사용하지 않는다. 과거 숫자는 진단 비교용으로 보존한다.
- 모든 reward의 이번 보정에 따른 추가 성능 판정은 INCONCLUSIVE: 이 점수쌍만으로 어떤 reward나 생존/추종 인수가 원인인지 식별하지 못한다. 새 학습 승인 없음.
- 근거: H1_OFFICIAL_CALIBRATION_20260910.md, workspace/calibration/h1_official_20260910/H1_OFFICIAL_CALIBRATION.json. 공식점수와 proxy 수식을 혼동하지 않는다.
