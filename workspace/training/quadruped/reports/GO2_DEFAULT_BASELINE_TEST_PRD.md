...
> **⚠️ CLOSED (260905, G-D94).** 이 문서는 v1 범위(Default-01 vs Pilot-01 쌍대 비교)에서
> 공식 종료됐다. Chain-01 계보(G-A009 이후)는 이 PRD를 갱신하지 않고 진행됐으며
> (`GO2_PROJECT_STATE.md` G-F142), 이후 이력을 이 문서에 소급 기입하지 않는다.
> **Go2의 유일한 living 정본은 이제 `GO2_PROJECT_STATE.md`다.** 아래 본문은 v1 당시
> 시점의 기록으로만 보존한다 — 현재 단계·기준선·다음 행동은 `GO2_PROJECT_STATE.md`
> 최신 섹션을 따른다(이 문서의 "0. 예선 기준 현재 위치"는 갱신되지 않은 과거 스냅샷이다).
