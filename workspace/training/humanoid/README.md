# Humanoid (H1) 학습 — NAVER Connect Robotics Cup 2026 예선

휴머노이드 H1 의 보행 정책 학습 패키지. **reward 수정 → 학습 → 영상으로 검증 → 제출** 단계.
...
**`humanoid_rewards.py`** 의 `REWARD_WEIGHTS` 안 값들을 자유롭게 바꿔 실험합니다.
9개 항목은 `[이동·등반]` / `[자세·안정]` 두 그룹으로 묶여 있고, 전부 조정 대상이에요.
어떤 reward 가 뭘 하는지는 파일 안 주석과 예선 가이드를 참고하세요. 그 외(`h1_task/`, `train.py`, `play.py`) 및 이 파일의 다른 코드는 건드리지 마세요.
...
- `--max_iterations`: 길수록 좋지만 수렴하면 정체. RTX 5080 기준 10000 iter ≈ 5h
  (H1 이 Go2 보다 step 당 계산은 가볍습니다 — 이유는 아래 §학습 속도 참고).
...
전체 로그·중간 체크포인트: `humanoid/logs/rsl_rl/humanoid/<run>/`

### 이어서 학습 (resume) — 끊긴 학습 복구 / 추가 학습

이미 돌린 run 의 마지막 지점에서 **가중치를 이어받아** 더 학습합니다 (세션 끊김 복구, "조금 더 돌려보기").
평소 학습 명령에 **`--resume`** 만 붙이면 가장 최근 run 을 이어받아요 — tmux 안에서:
...
```bash
ls logs/rsl_rl/humanoid/        # run 폴더 목록 확인 (예: 2026-06-19_10-00-00)
...
- `--load_run <폴더명>` : `logs/rsl_rl/humanoid/` 안의 run 폴더 이름 (생략 = 가장 최근).
- `--checkpoint model_<iter>.pt` : 그 run 의 특정 체크포인트 (생략 = 마지막).
- 이어서 학습한 결과는 **새 run 폴더**에 쌓이고, 끝나면 평소처럼 best 가 `exported/` 로 정리됩니다.

**`logs/` 가 통째로 날아갔을 때 (서버 재시작 등) — `exported/model_best.pt` 로 이어받기**

`--load_run`/`--checkpoint` 는 `logs/<run>/` 폴더 안에서 모델을 찾기 때문에, 살려둔 best 모델을 *가짜 run 폴더*에 한 번 넣어주면 됩니다 (다른 PC 에서 받은 `model_best.pt` 도 똑같이):

```bash
cd /workspace/training/humanoid
mkdir -p logs/rsl_rl/humanoid/from_best
cp exported/model_best.pt logs/rsl_rl/humanoid/from_best/
...
- ⚠️ 혹시 `model_best.pt` 를 못 찾는다고 나오면 **숫자 이름**으로 바꿔 넣으세요 — `cp exported/model_best.pt logs/rsl_rl/humanoid/from_best/model_99999.pt` 후 `--checkpoint model_99999.pt` (이어받을 iter 번호는 파일 내부 값이라 이름은 아무 숫자나 OK).
...
### 플레이 후 생기는 파일 — `humanoid/exported/`
...
```bash
cd /workspace/training/humanoid
/workspace/IsaacLab/isaaclab.sh -p play.py --task Humanoid-v0 --num_envs 16 \
    --headless --video --video_length 1000 --enable_cameras \
...
### 밀침 테스트 — H7(밀침 회복) 대비 확인 (`--push`)

채점표의 **H7 밀침 회복**을 영상으로 확인하고 싶을 때 씁니다. 4초마다 무작위
...
| 제출물 | 위치 / 방법 | 설명 |
|---|---|---|
| `policy.pt` | `humanoid/exported/` | 학습한 정책 (TorchScript) — **`play.py` 를 돌려야 생깁니다** |
| `env.yaml` | `humanoid/exported/` | 어떤 reward 로 학습했는지 (의도) |
...
> 📄 `exported/report.html` (결과 분석 리포트)은 **제출물이 아닙니다.**
> - 자기 학습 결과를 점검하도록 제공되는 참고 자료입니다.

---

## H1 특징

- **이족보행** — 균형이 사족보다 까다로워 reward 튜닝에 민감합니다. (예: `feet_air_time` 을 너무 키우면 한 발로만 깡총 뛰는 보행을 학습 — 파일 주석 참고.)
- 학습 지형의 박스 높이가 **0.15~0.50m** 로, 0.45m 박스언덕 step-up 까지 노출해 학습합니다. (사족 Go2 는 ≤0.15m 로 제한 — 박스언덕은 H1 의 영역.)
- **학습 속도는 빠른 편이에요.** IsaacLab 이 H1 을 단순화 콜라이더(`H1_MINIMAL_CFG`)로 돌려 step 당 충돌 계산이 가볍기 때문이에요. (사족 Go2 는 풀 콜라이더(`UNITREE_GO2_CFG`)라 더 느립니다.) 관절 수는 H1 이 많지만, 물리 시뮬 속도는 관절 수가 아니라 충돌 계산량이 좌우하거든요.
